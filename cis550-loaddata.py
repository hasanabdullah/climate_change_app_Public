...
climatedata_dir = 'C:\CIS550Data\ghcnd_hcn-002\ghcnd_hcn\TEST'
stormdetails_dir = 'C:\CIS550Data\Data\storm_details\extracted\TEST'
stormlocations_dir = 'C:\CIS550Data\Data\storm_locations\extracted\TEST'
...
def load_usstates_table(conn):
    ...
    conn.commit()
    print("US States Table load complete.")
...
def load_stations_table(conn):
    ...
    # commit changes to the db so it actually does something
    conn.commit()
    print("Station Table load complete.")
...
def load_climatedata_table(conn):
    ...
    for f in files:
        ...
        with open(filesdir+'/'+f) as thisfile:
            ...
            while line:
                ...
                #print("values for |"+ID+"| |"+str(YEAR)+"| |"+str(MONTH)+"| |"+ELEMENT+"|")
    
                
                if ((ELEMENT == 'PRCP') or (ELEMENT == 'SNOW') or (ELEMENT == 'SNWD') or 
                    (ELEMENT == 'TMAX') or (ELEMENT == 'TMIN') or (ELEMENT == 'TAVG') or (ELEMENT == 'TOBS')) :
                    ...
                    for i in range(1,32):
                        ...
                        #do not add to the dictionary if the value is "null" aka -9999
                        if (V != -9999):
        ...
        for key, values in climate_dict.items():
            ...
            counter += 1
            if counter%10000 == 0:
                print("Loading line #{}".format(counter))
            # insert into table
    ...
    # commit changes to the db so it actually does something
    conn.commit()
    print("Climate Table load complete.")
...
def load_stormdetails_table(conn):
    ...
    # create table
    cur.execute(
        ("CREATE TABLE {} (year INT, month_name VARCHAR(20), begin_date_time VARCHAR(20), end_date_time VARCHAR(20), cz_timezone VARCHAR(10), episode_id INT NOT NULL, event_id INT NOT NULL, event_type VARCHAR(50),"+
                                "injuries_direct INT, injuries_indirect INT, deaths_direct INT, deaths_indirect INT, damage_property VARCHAR(20), damage_crops VARCHAR(20), magnitude REAL, magnitude_type VARCHAR(5),"+
                                "flood_cause VARCHAR(50), episode_narrative TEXT, event_narrative TEXT, state VARCHAR(50),"+
                                "PRIMARY KEY(event_id, episode_id))").format(table_name) )
    #TODO: Add in the foreign reference to the states, but also need to change the state value from full name to abbreviation
    #TODO: Convert the BEGIN_DATE_TIME and END_DATE_TIME to DATETIME types 
    #NOTE: DAMAGE_PROPERTY and DAMAGE_CROPS are not numbers, they have a 'K' or 'M' after the number, consider cleaning this up
    
    cur.execute(
        ("CREATE TABLE {} (event_id INT NOT NULL, episode_id INT NOT NULL, tor_length REAL, tor_width REAL, tor_f_scale VARCHAR(3),"+
    ...
    #TODO: Convert the states to code and create a foreign key to the states table 
    #TODO: For some reason I am struggling to make the episode_id a foreign key to the storm_details table
    ...
    for f in files:
        ...
        with open(filesdir+'/'+f, encoding="utf-8") as thisfile:
            ...
            while line:
                ...
                #TODO: Need to change all the floods to one type of flood
    ...
    # commit changes to the db so it actually does something
    conn.commit()   
    print("Storm Details Table & Tornados Details Table load complete.") 
...
def load_stormlocations_table(conn):
    ...
    # create table
    cur.execute(
        ("CREATE TABLE {} (event_id INT NOT NULL, episode_id INT NOT NULL, location_index INT, center_range REAL,"+
    ...
    # commit changes to the db so it actually does something
    conn.commit()   
    print("Storm Locations Table load complete.") 
